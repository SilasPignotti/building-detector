...
from app.config import COLAB_SERVER_URL, MAX_CONTENT_LENGTH, UPLOAD_FOLDER
...
def process():
    ...
    try:
        ...
        satellite_images = [
            ...
            if f.startswith("satellite_")
        ]
        if not satellite_images:
            return jsonify({"error": "No satellite image found"}), 400

        satellite_images.sort(
            key=lambda x: os.path.getctime(get_file_path(x)), reverse=True
        )
        latest_image = satellite_images[0]
        image_path = get_file_path(latest_image)
        created_at = os.path.getctime(image_path)
        logger.info(f"Using most recent image: {image_path} (created: {created_at})")
        ...
        return send_to_colab_server(
            colab_url, image_path, latest_image, transformed_points
        )
...
def send_to_colab_server(colab_url, image_path, image_filename, transformed_points):
    ...
    try:
        ...
        if response.status_code != 200:
            error_msg = (
                f"Processing failed on Colab server (Status {response.status_code})"
            )
            try:
        ...
        return jsonify(
            {"success": True, "result_url": "/uploads/building_regularized.geojson"}
        )
...
def download():
    ...
    try:
        ...
        clean_geojson = {"type": "FeatureCollection", "features": []}
